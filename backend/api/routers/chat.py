...
async def ask(websocket: WebSocket):
    ...
    if model_name == ChatModels.paid and not user.can_use_paid:
        ...
        return
    if (model_name == ChatModels.gpt4 or model_name == ChatModels.gpt4_mobile or model_name == ChatModels.gpt4_browsing or model_name == ChatModels.gpt4_plugins) and not user.can_use_gpt4:
        await websocket.close(1007, "errors.userNotAllowToUseGPT4Model")
        return
    if model_name in [ChatModels.gpt4, ChatModels.gpt4_mobile, ChatModels.gpt4_browsing, ChatModels.gpt4_plugins, ChatModels.paid] and not config.get("chatgpt_paid", False):
    ...
    async with get_async_session_context() as session:
        ...
        if user.available_ask_count != -1 and user.available_ask_count <= 0:
            ...
            return
        if user.available_gpt4_ask_count != -1 and user.available_gpt4_ask_count <= 0 and (model_name == ChatModels.gpt4 or model_name == ChatModels.gpt4_mobile or model_name == ChatModels.gpt4_browsing or model_name == ChatModels.gpt4_plugins):
        ...
        if has_got_reply:
            async with get_async_session_context() as session:
                ...
                if user.available_ask_count != -1 or user.available_gpt4_ask_count != -1:
                    ...
                    if (model_name == ChatModels.gpt4 or model_name == ChatModels.gpt4_mobile or model_name == ChatModels.gpt4_browsing or model_name == ChatModels.gpt4_plugins) and user.available_gpt4_ask_count != -1:
